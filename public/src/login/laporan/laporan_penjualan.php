...
$sql = "SELECT 
            ...
            GROUP_CONCAT(dp.subtotal SEPARATOR ', ') AS subtotal
        FROM penjualan p
            ...
            <table class="w-full border-collapse table-auto text-sm text-gray-600">
                <thead>
                    <tr class="bg-gray-200 text-gray-700">
                        ...
                        <th class="px-6 py-3 text-left">Potongan Harga</th>
                    ...
                    <?php if (mysqli_num_rows($result) > 0) : ?>
                        <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                            <tr class="border-b hover:bg-gray-100">
                                ...
                                <td class="px-6 py-4"><?= htmlspecialchars($row['nama_pelanggan']); ?></td>
                                <td class="px-6 py-4"><?= htmlspecialchars($row['nama_produk']); ?></td>
                                <td class="px-6 py-4"><?= htmlspecialchars($row['jumlah_produk']); ?></td>
                                <td class="px-6 py-4">
                                    <?php
                                    $subtotalArray = explode(',', $row['subtotal']);
                                    $formattedSubtotal = array_map(function($value) {
                                        return number_format((float)trim($value), 0, ',', '.');
                                    }, $subtotalArray);
                                    echo implode(', ', $formattedSubtotal);
                                    ?>
                                </td>
                                <td class="px-6 py-4"><?= number_format($row['total_harga'], 0, ',', '.'); ?></td>
                                <td class="px-6 py-4"><?= number_format($row['potongan_harga'], 0, ',', '.'); ?></td>
                                <td class="px-6 py-4"><?= number_format($row['total_bayar'], 0, ',', '.'); ?></td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else : ?>
                        <tr>
